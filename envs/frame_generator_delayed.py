...
# =============================================================================
# Custom Gym environment: FrameGeneratorDelayed
# This environment presents a frame extracted from a coherent video sequence to the agent at each timestep.
# It is used to train the agent on a delayed timing task where the timing task starts after a cue frame
# It is used to Simulate human timing experiments where participants start timing after a visual cue.
# The goal is to analyse the changes in neural dynamics of the DRL agent before and during the timing task
# =============================================================================
class FrameGeneratorDelayed(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        """
        Reset environment state, select a random starting point in the video,
        insert the cue frame, and initialize observation and info.
        """
        ...
        # Randomly select position for cue frame (red frame indicating start of timing task)
        self.cue_frame_position = random.randint(int(len(self.video_frames) / 3),
                                                 (len(self.video_frames) - (4 * self.target_duration)))
        ...
        self.info = {"action": 0, "current_frame_index": 0, "timestep": self.timestep,
                     "cue_frame_position": self.cue_frame_position}
        return self.state, self.info

    def step(self, action):
        """
        Apply the agent's action, advance the frame index, compute reward,
        and determine if episode is terminated.
        """

        self.current_frame_index += 1
        # Increment timestep only after the cue frame has been shown. timestep variable is used to keep track of target interval
        ...
        # Reset timestep at the end of each target interval
    ...
    def compute_reward2(self, target_duration, info):
        """
        Reward function:
        - Before cue frame: reward = 0
        - After cue frame:
            - If action = 0 (no action) and timestep < target_duration -> reward 0
            - If action = 0 and timestep >= target_duration -> reward -1
            - If action = 1 and timestep == target_duration -> reward 1
            - Otherwise reward = -1
        """
    ...
    def compute_terminated2(self, target_duration, info):
        # Episode ends when we have processed target_duration * n timesteps.
